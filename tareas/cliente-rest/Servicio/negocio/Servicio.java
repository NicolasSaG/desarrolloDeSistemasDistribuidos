  ...
  public Response alta(@FormParam("usuario") Usuario usuario) throws Exception {
    ...
    try {
      ...
      PreparedStatement stmt_4 = conexion
          .prepareStatement("select id_usuario from usuarios order_by id_usuario desc limit 1");
      try {
        ResultSet rs = stmt_1.executeQuery();
        try {
          if (rs.next()) {
            id_usuario = new Integer(rs.getInt(1));
          }
        } finally {
          rs.close();
        }
    ...
    if (id_usuario == null) {
      return Response.status(400).entity(j.toJson(new String("error al obtener el id_usuario"))).build();
    }
  ...
  public Response consulta(@FormParam("id_usuario") Integer id_usuario) throws Exception {
    ...
    try {
      PreparedStatement stmt_1 = conexion.prepareStatement(
          "SELECT a.email,a.nombre,a.apellido_paterno,a.apellido_materno,a.fecha_nacimiento,a.telefono,a.genero,b.foto FROM usuarios a LEFT OUTER JOIN fotos_usuarios b ON a.id_usuario=b.id_usuario WHERE usuarios.id_usuario=?");
      try {
